...
### 6.1 动力学仿真可视化
**1. 动力学瞬态应力云图 (位移放大 1,500 倍)**
![vm_cloud_frame](docs/images/supervisor/vm_cloud_frame_100.png)
*   **物理说明**：该图展示了结构在谐波载荷（Harmonic Load）作用下 $t=1.28s$ 时的瞬时响应。载荷时程完整覆盖了仿真全过程（0-2.0s）。
*   **可视化增强**：为了在保持结构形态合理性的同时呈现亚毫米级变位，图中应用了 **1,500 倍** 变形放大。灰色虚线描述了结构的初始外形，彩色云图揭示了由节点载荷激发的整体受力与振型。

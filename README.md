...
### 2.1 参数化建模与前处理
模型支持通过 YAML 格式定义几何拓扑、材料属性（$E, \rho, \nu$）及边界约束。
*   **相关文件**: 
...
### 3.3 训练成果分析
通过对 10,000 个动力学样本的离线训练，模型在独立测试集上的表现如下：

| 指标维度 | 数值表现 | 初步结论 |
| :--- | :--- | :--- |
| **MAE (平均绝对误差)** | **0.076** | 模型能够较好地回归出大部分杆件的损伤程度。 |
| **F1-Score (损伤定位)** | **0.384** | 具备对显著损伤位置进行初步判定的能力。 |

![训练历史](docs/images/dl_results/gt_history.png)
*   **结果说明**: 训练历史曲线显示模型有效地学习到了动力学信号与结构刚度分布之间的内在映射关系。

...
| :--- | :--- | :--- |
| **收敛速度** | **极快**。在 10 轮内 MAE 即可降至 0.01 左右。 | 较慢。由于需要权衡物理 Loss，初期波动较大。 |
...
### 4.2 核心发现
1.  **数据红利**：将数据集扩展至 10,000 样本后，GT 模型的 F1-Score 较 2,000 样本时提升了约 15%，表明图神经网络能有效吸收大规模动力学特征。
2.  **空间关联性**：GT 模型展现出了明显的“空间局部性”，即靠近传感器节点的单元损伤识别精度显著高于远端单元，这符合力学信号衰减规律。
3.  **计算效率**：得益于简化的 GAT 实现，GT 在推理阶段的延迟极低（< 1ms/sample），能够满足实时健康监测的需求。

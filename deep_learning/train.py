...
def main():
    ...
    parser.add_argument('--model', type=str, default='both', choices=['lstm', 'pinn', 'both'])
    ...
    if args.model in ['lstm', 'both']:
        ...
        lstm_model = LSTMDamagePredictor(
            input_dim=input_dim,
        ...
        lstm_model.load_state_dict(
            torch.load(os.path.join(ckpt_dir, 'lstm_best.pth'), map_location=device)
        )
        lstm_test_metrics, lstm_test_pred, lstm_test_true = evaluate(
            lstm_model,
            test_loader,
            nn.MSELoss(),
            device,
        ...
        prediction_artifacts['lstm_pred'] = lstm_test_pred.astype(np.float32)
        prediction_artifacts['true_damage'] = lstm_test_true.astype(np.float32)
        
        results['lstm'] = {
            'history': lstm_history,
            'test_metrics': lstm_test_metrics
        }
        print(f"\nLSTM Test Results: MAE={lstm_test_metrics['mae']:.4f}, F1={lstm_test_metrics['f1']:.4f}")
    ...
    if args.model == 'both':
        ...
        print("="*50)
        print(f"{'Metric':<15} {'LSTM':<15} {'PINN':<15}")
        print("-"*45)
        for metric in ['mae', 'rmse', 'f1', 'iou']:
            lstm_val = results['lstm']['test_metrics'].get(metric, 0)
            pinn_val = results['pinn']['test_metrics'].get(metric, 0)
            print(f"{metric:<15} {lstm_val:<15.4f} {pinn_val:<15.4f}")
